...
하지만 자바스크립트는 <strong>function-level scope</strong>를 사용한다. function-level scope란 함수 내에서 선언된 변수와 매개 변수는 함수 내에서만 유효하고 함수 외부에서는 유효하지 않다(참조할 수 없다)는 것이다.

단, ECMAScript 6에서 도입된 [let](https://developer.mozilla.org/ko/docs/Web/JavaScript/Reference/Statements/let) keyword를 사용하면 <strong>block-level scope</strong>를 사용할 수 있다.
