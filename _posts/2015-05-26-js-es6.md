...
### 3.3.1 Multiline Strings (복수행 문자열)
...
### 3.3.2 String Interpolation (문자열 삽입)
...
## 3.4 Extended Parameter Handling (함수 파라미터 확장)

### 3.4.1 Default Parameter value (기본 파라미터 초기값)
...
### 3.4.2 Rest Parameter (Rest 파라미터)
...
### 3.4.3 Spread Operator (Spread 연산자)
...
함수에 배열 인자를 전달할 때에 apply()를 사용할 필요가 없다.
...
## 3.5 Destructuring (디스트럭처링)

Destructuring은 객체 또는 배열에 저장되어 있는 여러 값을 추출해내는 매우 편리한 방법이다.

### 3.5.1 Object destructuring (객체 디스트럭처링)
...
### 3.5.2 Array destructuring (배열 디스트럭처링)
...
constructor 메서드는 생략할 수 있다. constructor를 생략하면 객체의 생성과 동시에 초기화는 할 수 없다.

```javascript
class Foo {}

console.log(new Foo(1)); // Foo {}
...
### 3.6.3 Class Inheritance (클래스 상속)
