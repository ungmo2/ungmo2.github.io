...
자바스크립트는 선언문을 모두 호이스팅(Hoisting)한다. 하지만 let 키워드로 선언된 변수는 <strong>Hoisting되지 않는다</strong>.
...
const는 let과 대부분 동일한 특징을 갖는다. let은 초기화 이후 재할당이 자유로우나 const는 초기화 이후 재할당이 금지된다.
...
- 변경이 발생하지 않는 primitive형 변수와 참조형 변수에는 const를 사용

참조형 변수에 const를 사용하는 이유는 참조형 변수에 저장되는 주소값은 변경될 일이 없기 때문이다. 자바스크립트의 값은 대부분 객체(primitive형 변수를 제외한 모든 값은 객체이다)이므로 결국 대부분의 경우 const를 사용하게 된다.
