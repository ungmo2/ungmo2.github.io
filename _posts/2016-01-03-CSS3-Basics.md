...
모든 HTML 요소는 박스와 같은 모양을 가지고 있다. CSS 박스 모델은 모든 HTML 요소를 감싸고 있는 margin, border, padding 속성을 의미한다.
...
`max-width` 속성을 사용하면 브라우저 너비가 요소의 너비보다 좁아지면 자동으로 요소의 너비가 줄어든다.
...
## 4.6 글자 (Text)와 폰트 (Font)

## 4.7 위치 (Position)

## 4.8 float

## 4.9 Shadow / Rounded Corner / Gradients
