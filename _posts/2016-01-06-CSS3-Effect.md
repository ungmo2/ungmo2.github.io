...
bottom clip color crop font-size font-weight
...
또한 요소의 속성이 변화하면 브라우저는 변화에 영향을 받는 모든 요소의 기하값(위치와 크기)를 계산하여 layout 작업을 수행해야 한다. 이것은 브라우저에게 스트레스를 주어 성능에 영향이 미치게 된다. 심지어 다수의 자식 요소를 가지고 있는 요소(예를 들어 body 요소)가 변경되면 모든 자식 요소의 기하값이 재계산될 수 도 있다. 따라서 layout에 영향을 주는 트랜지션 효과는 회피하기 위한 노력이 필요하다.
...
CSS3 애니메이션은 자바스크립트를 사용하지 않더라도 보다 부드러운 애니메이션 효과를 만들 수 있다는 장점이 있으나 경우에 따라서는 자바스크립트를 사용하는 것이 나을 수도 있다. jQuery 등의 애니메이션 기능은 CSS보다 간편하게 애니메이션 효과를 가능케 한다.
