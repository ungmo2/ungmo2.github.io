...
애니메이션의 시작과 끝에 어떤 스타일을 적용할지 설정한다. 애니메이션이 어떻게 나타나고 어
떻게 사라질지 효과를 넣는 속성이다. 예를 들어 -webkit-animation-delay 속성으로 애니메
이션 재생이 지연되고 있을 때에는, 지연되는 동안 애니메이션의 첫 프레임에 설정한 스타일을
볼 수 없다. 하지만 -webkit-animation-fill-mode 속성으로 backward나 both를 설정하면
지연 시간에 상관없이 애니메이션의 첫 프레임에 설정한 스타일을 볼 수가 있다




"none"(초기 값)으로 설정하면 애니메이션이 완료된 후에 애니메이션 요소가 원래 스타일 속성으로 되돌아갑니다. 이 속성을 "forwards" 값으로 설정하면 애니메이션이 완료될 때 애니메이션의 마지막 값("to" 또는 "100%" 키 프레임에 지정된 값)이 적용되도록 지정합니다.
