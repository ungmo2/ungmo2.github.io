...
애니메이션 효과는 HTML 요소에 적용되는 CSS 스타일을 다른 CSS 스타일로 부드럽게 변화시킨다. 애니메이션은 애니메이션을 나타내는 CSS 스타일과 애니메이션의 중간 상태를 나타내는 키프레임으로 이루어진다.
...
- 세밀한 제어를 위해서는 자바스크립트를 사용한다. 현재 마우스의 포지션을 동적으로 추적하는 기능은 자바스크립트가 훨씬 유용하다.
