...
```
선택자 { text-shadow: Horizontal-Length Vertical-Length Blur-Radius Shadow-Color; }
```

| 속성값                   | 설명                | 생략   |
|:-----------------------|:-------------------|:------|
| Horizontal-Length (px) | 그림자를 텍스트의 오른쪽으로 지정값만큼 이동시킨다
| Vertical-Length (px)   | 그림자를 텍스트의 아래로 지정값만큼 이동시킨다
| Blur-Radius (px)       | 그림자의 흐림정도를 지정한다. 지정값만큼 그림자가 퍼져 보이게 된다. | 생략 가능
| Shadow-Color (color)   | 그림자의 색상을 지정한다 | 생략 가능
