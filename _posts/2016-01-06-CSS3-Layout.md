...
logo image를 포함하는 a tag(#logo)의 height를 logo image와 같은 height인 36px로 지정하고 상하 margin을 12px씩 부여하면 logo 요소의 높이는 60px이 되고 header의 높이와 같아져 이미지는 수직 정렬된다. a tag는 inline 요소이므로 margin을 정의하기 위해서 `display: inline-block;`을 설정한다. 또한 img tag에 부여한 height 속성을 css로 옮긴다.
