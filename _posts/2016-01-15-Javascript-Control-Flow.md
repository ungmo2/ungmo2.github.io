...
switch 문의 경우, `switch`변수의 값과 일치되는 `case`구문으로 실행 순서가 이동하게 된다. `break` keyword는 switch 구문에서 탈출하는 역할을 수행한다.
...
흐름제어를 위해서는 조건식을 평가하여 논리적 참, 거짓을 구별하여 의사결정을 하는 것이 일반적이다. 예를 들어 점수가 80점 이상이면 합격이고 80 미만이면 불합격이라는 기준이 있을 때, 80이상이면 참으로 평가하여 흐름을 제어할 수 있다.
