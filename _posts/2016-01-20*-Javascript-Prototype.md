...
ECMAScript spec에서는 *자바스크립트의 모든 객체는 자신의 프로토타입을 가리키는 [[Prototype]]이라는 숨겨진 프로퍼티을 가진다* 라고 되어있다. 크롬 브라우저에서는 `__proto__`가 바로 숨겨진 [[Prototype]] 속성을 의미한다. 즉 student 객체는 `__proto__`라는 숨겨진 속성에 자신의 부모 객체(프로토타입)를 Link하고 있는 것이다.
