...
두 객체의 근본적 차이는 `numbers`는 `Array.prototype`을 상속받았으나 `numbers_object`는 `Object.prototype`을 상속 받았다는 것이다. `Array`객체는 다양한 메서드(e.g. `sort`)와 속성(e.g. `length`)을 제공한다.
...
객체가 동적으로 속성을 추가할 수 있는 것처럼 배열도 동적으로 요소를 추가할 수 있다.
