...
category: css
section: css
description: CSS3 Syntax 선택자 (Selector) 프로퍼티 (Property / 속성) 값 (Value / 속성값) HTML과 CSS의 연동 Link Embedding Inline style Reset CSS 사용하기 CSS Versions
---
...
CSS는 HTML 요소의 style(design, layout etc)을 정의하는데 사용된다. 그러기 위해서는 먼저 <strong>CSS는 스타일을 적용하고자 하는 HTML 요소(들)에 특정</strong>하고 적용되어야 한다.
...
위와 같은 문장을 Rule Set(또는 Rule)이라 하며 선택자에 의해 선택된 특정 HTML 요소를 어떻게 렌더링(Rendering)할 것인지 브라우저에 지시하는 역할을 한다. 위의 CSS Rule set은 HTML 문서에 속해 있는 선택자를 통해 모든 h1 요소를 선택한 후 선택된 h1 요소의 스타일을 선언 블록에서 정의하고 있다.

이와 같은 Rule Set의 집합을 스타일시트(Style Sheet)라 한다.

# 2. 프로퍼티 (Property / 속성)

선택자로 HTML 요소를 선택하고 {} 내에 프로퍼티(속성)와 속성값을 지정하는 것으로 다양한 style을 적용시킬수 있다. 프로퍼티는 [표준 스펙](https://www.w3.org/community/webed/wiki/CSS/Properties)으로 이미 지정되어 있는 것을 사용하여야하며 사용자가 임의로 정의할 수 없다. 여러개의 프로퍼티를 연속해서 지정할 수 있으며 세미콜론(;)으로 구분한다.
...
HTML 외부에 있는 CSS 파일을 HTML에서 로드하는 방식이다. 가장 일반적인 방식이다.
...
HTML 내부에 CSS를 포함시키는 방식이다. 매우 간단한 웹페이지의 경우는 문제될 것이 없겠지만 Link style을 사용하는 편이 좋다. HTML과 CSS는 구분되어 작성하고 관리되는 것이 일반적이다.
...
태그 내부에 CSS를 기술하는 방식이다. JavaScript가 동적으로 CSS를 생성할 때 사용하는 경우가 있다. 하지만 일반적인 경우 Link style을 사용하는 편이 좋다.
