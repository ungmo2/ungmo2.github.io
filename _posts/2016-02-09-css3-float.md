...
부모 요소에 사전에 작성한 clearfix 클래스만 추가하면 되기 때문에 가장 깔끔하고 간편하므로 이 방법을 사용하는 것을 추천한다.

또 다른 방법은 float 프로퍼티 대신 `display: inline-block;`을 설정하는 것이다. 주의해야야 점은 [inline-block 프로퍼티](./css3-display#inline-block-) 요소를 연속 사용하는 경우, 좌우에 정의하지 않은 space(4px)가 자동 지정되는 것이다. 이것을 회피 방법은 [Fighting the Space Between Inline Block Elements](https://css-tricks.com/fighting-the-space-between-inline-block-elements/)를 참조하기 바란다.
