...
이벤트 위임(Event Delegation)은 다수의 자식 노드에 이벤트 리스너를 등록하는 것 대신 하나의 부모 노드에 이벤트 리스너를 등록하는 방법이다. 위의 경우 6개의 자식 노드에 각각 벤트 리스너를 등록하는 것 대신 부모 노드(ul#parent-list)에 이벤트 리스너를 등록하는 것이다.

[이벤트 흐름]()이 버블링의 경우이벤트는

이때 이벤트 리스너는 이벤트 버블링(Event Bubbling)에 의해 해당되는 자식 요소를 찾게된다.
