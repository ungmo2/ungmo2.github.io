...
JSONP(JSON with Padding)은 도메인이 다른 서버로 데이터를 요청할 때 사용한다.

기본적으로 웹브라우저는 도메인이 다른 서버로 접근하지 못하게 제한하고 있는데, 이것을 동일출처원칙(Same-origin policy)이라고 합니다. 그러나 실무에서는 부득이 다른 도메인에 연결된 서버로 데이터를 요청해야만 하는 상황을 만나게 됩니다. JSONP는 바로 이러한 경우에 동일출처원칙을 회피하는 일종의 편법 입니다.
