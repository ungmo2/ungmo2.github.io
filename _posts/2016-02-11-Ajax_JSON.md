...
| data        | http 요청 후 반환되는 데이터
| dataType    | http 요청 후 반환되는 데이터의 type (xml, html, json, jsonp, text)
| async       | http 요청 시 동기화 유무
| timeout     | http 요청 제한 시간
| success     | http 요청 성공 이벤트 핸들러
| error       | http 요청 실패 이벤트 핸들러
| complete    | http 요청 완료 이벤트 핸들러
...
<html>
  ...
  <body>
    ...
    $.getJSON("http://ungmo2.github.io/files/data-jsonp.js?callback=?");
    </script>
  </body>
</html>
```
