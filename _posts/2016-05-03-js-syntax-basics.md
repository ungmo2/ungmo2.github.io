...
데이터 타입이란, 변수가 할당받을 값의 타입을 말한다. 변수명은 메모리에 확보된 공간을 가리키는 식별자(identifier)이다.

**리터럴(literal)이란, 프로그램 안에서 직접 만들어 낸 상수 값 자체를 의미한다.** 리터럴은 메모리 공간에 저장되며 리터럴이 할당된 변수는 리터럴이 저장된 메모리의 주소를 가리킨다.
