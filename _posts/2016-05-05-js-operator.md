...
위와 같이 **표현식은 평가(Evaluation)되고 그 결과, 하나의 값으로 수렴된다.** 즉, **표현식은 하나의 값으로 평가될 수 있는 문(Statement)**이다. 위 예제는 하나의 표현식으로만 이루어져 있지만 하나 이상의 표현식을 결합하여 하나의 값을 만들어낼 수 있다.
...
산술 연산자는 피연산자가 숫자이거나 숫자로 타입을 변환할 수 있는 값을 산술 연산한다. 연산을 할 수 없거나 피연산자를 숫자로 타입 변환할 수 없는 경우 NaN을 반환한다.
...
이항 산술 연산자는 부수 효과가 없다. 다시 말해, 피연산자의 값을 변경하지 않는다. 하지만 단항 산술 연산자는 모두 부수 효과가 있어서 피연산자의 값이 변경된다.
...
할당 연산자는 우항의 피연산자의 값을 좌항의 변수에 할당한다. 할당 연산자는 좌항의 변수에 값을 할당하므로 부수 효과가 있는 연산자이다.
...
표현식은 "하나의 값으로 평가될 수 있는 문"이라고 정의했다. 그렇다면 할당 연산은 표현식일까? 아래의 예제를 살펴보자.
...
비교 연산자는 우항의 값과 좌항의 값을 비교하여 불리언 값을 반환한다. 부수 효과는 없다.
...
논리 연산자는 우항과 좌항의 불리언 값을 피연산자로 사용하여 논리 연산을 수행하여 불리언 값을 반환하는 것이 일반적이다. 일반적이라고 표현한 것은 반드시 불리언 값을 반환하지 않을 수 있다는 것이다. 이에 대해서는 다음의 "단축 평가"에서 설명하기로 하고 지금은 일반적인 논리 연산자에 알아보도록 하자.
...
**논리 연산자는 사실 피연산자 중 하나를 반환한다.** 따라서 논리 연산자가 피연산자가 불리언 값이 아닌 경우, 피연산자의 값을 그대로 반환한다. 논리 연산자는 다음의 규칙을 따라서 "단축 평가"로 검사된다.
