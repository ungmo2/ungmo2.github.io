...
논리 연산자(Logical Operator)는 우항과 좌항의 피연산자 값을 논리 연산하여 불리언 값을 반환하는 것이 일반적이다. 일반적이라고 표현한 것은 반드시 불리언 값을 반환하지 않을 수 있다는 것이다. 이에 대해서는 다음의 "단축 평가"에서 설명하기로 하고 지금은 일반적인 논리 연산자에 알아보도록 하자.
