...
개발자의 의도와는 상관없이 자바스크립트 엔진에 의해 암묵적으로 타입이 자동 변환되기도 한다. 이를 **암묵적 타입 강제 변환(implicit coercion) 또는 타입 강제 변환(type coercion)**이라고 한다. 암묵적 타입 강제 변환은 자바스크립트 엔진이 코드를 평가하는 과정에서 발생한 부수 효과(side effect)로 인해 자동 발생한다.
