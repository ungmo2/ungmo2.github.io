...
기본자료형의 값은 값(value)으로 전달된다. 즉, 값이 복사되어 전달된다. 이를 pass-by-value(값으로 전달)라 한다. 기본자료형은 값이 한번 정해지면 변경할 수 없다.(immutable) 또한 이들 값은 메모리의 스택 영역(Stack Segment)에 고정된 메모리 영역을 점유하고 저장된다.
