...
**스코프는 함수를 호출할 때가 아니라 함수를 어디에 선언하였는지에 따라 결정된다. 이를 렉시컬 스코핑(Lexical scoping, Static scoping)라 한다.** 아래 예제의 함수 bar가 어디에서 호출될 지 선언 시점에서는 알 수 없다. 따라서 함수를 어디서 호출하는지에 따라 스코프를 결정하지 않고 선언한 위치에 따라 스코프를 결정한다.

아래 예제의 foo 함수와 bar 함수의 렉시컬 스코프는 전역 스코프와 자신의 스코프이다.
...
아래 예제의 경우, foo 함수의 렉시컬 스코프는 전역과 자신의 스코프이고 bar 함수의 렉시컬 스코프는 전역, foo 함수, 그리고 자신의 스코프이다.
