...
생성된 함수 객체는 `[[Scopes]]` 프로퍼티를 가지게 된다. [[Scopes]] 프로퍼티는 현재 실행 컨텍스트의 스코프 체인이 참조하고 있는 객체를 값으로 설정한다.
...
이때 알 수 있는 것은 함수선언식의 경우, 함수명을 프로퍼티로 함수 객체가 할당된다는 것이다. 즉 함수선언식도 함수표현식과 동일하게 변수에 함수를 할당한다. 단, 함수선언식은 변수 객체(VO)에 함수명을 변수명으로 추가하고 즉시 함수 객체를 할당하지만 함수 표현식은 일반 변수의 방식을 따른다. 따라서 함수선언식의 경우, 선언문 이전에 함수를 실행할 수 있다. 이러한 현상을 **[함수 호이스팅(Function Hoisting)](./js-function#2-함수-호이스팅function-hoisting)**이라 한다.
...
그 후, Caller(전역 컨텍스트)의 [[Scope]] 프로퍼티가 참조하고 있는 객체가 스코프 체인에 push된다. 따라서, 이 경우 함수 foo를 실행한 직후 실행 컨텍스트의 스코프 체인은 Activation Object(함수 foo의 실행으로 만들어진 AO-1)과 전역 객체를 순차적으로 참조하게 된다.
