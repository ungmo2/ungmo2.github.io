...
배열을 순회하며 각 요소에 대하여 인자로 주어진 콜백함수를 실행하고 반환값(결과값)으로 새로운 배열을 반환한다. 이때 원본 배열은 변경되지 않는다. IE 9 이상에서 정상 동작한다.
...
배열 내 일부 요소가 콜백함수의 테스트를 통과하는지 확인하여 그 결과를 blooean으로 반환한다. IE 9 이상에서 정상 동작한다.
...
배열 내 모든 요소가 콜백함수의 테스트를 통과하는지 확인하여 그 결과를 반환한다. IE 9 이상에서 정상 동작한다.
