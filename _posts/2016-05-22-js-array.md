...
section: javascript
description: 배열(array)는 1개의 변수에 여러 개의 값을 순차적으로 저장할 때 사용한다. 자바스크립트의 배열은 객체이며 유용한 내장 메서드를 포함하고 있다.
---
...
두 객체의 근본적 차이는 `numbersArr`는 `Array.prototype`을 상속받았으나 `numbersObject`는 `Object.prototype`을 상속 받았다는 것이다. `Array` 객체는 다양한 메서드(e.g. `sort`)와 프로퍼티(e.g. `length`)를 제공한다.
...
ES6의 [Arrow function](./es6-arrow-function)를 사용하면 this를 Prefixer.prototype에 bind하기 때문에 두번째 인자로 this를 생략하여도 동일한 동작을 한다.
...
ES6의 [Arrow function](./es6-arrow-function)를 사용하면 this를 Counter.prototype에 bind하기 때문에 두번째 인자로 this를 생략하여도 동일한 동작을 한다.
