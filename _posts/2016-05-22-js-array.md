...
0개 이상의 값을 쉼표로 구분하여 대괄호([])로 묶는다. 첫번째 값은 인덱스 '0'으로 읽을 수 있다. 존재하지 않는 요소에 접근하면 `undefined`를 얻게 된다.
...
## 5.2 Array.prototype.concat(item...)
...
## 5.8 Array.prototype.slice(start, end)
...
## 5.9 Array.prototype.splice(start, deleteCount, item…)
...
## 5.10 Array.prototype.sort(comparefunc)
...
ES6의 [Arrow function](./js-es6#arrow-function)를 사용하면 this를 Prefixer.prototype에 bind하기 때문에 두번째 인자로 this를 생략하여도 동일한 동작을 한다.
...
ES6의 [Arrow function](./js-es6#arrow-function)를 사용하면 this를 Counter.prototype에 bind하기 때문에 두번째 인자로 this를 생략하여도 동일한 동작을 한다.

## 5.13 Array.prototype.filter()

배열을 순회하며 각 요소에 대하여 인자로 주어진 콜백함수의 실행 결과가 true인 요소값만을 모은 새로운 배열을 반환한다. 콜백함수의 인자로 요소값, 요소 인덱스, 순회할 배열을 전달할 수 있다.
