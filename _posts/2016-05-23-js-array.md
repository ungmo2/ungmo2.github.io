...
length 프로퍼티는 요소의 개수(배열의 길이)를 나타낸다. 배열 인덱스는 32bit 양의 정수를 갖는다. 따라서 length 프로퍼티의 값은 양의 정수이며 2<sup>32</sup>(4,294,967,296) 미만이다.

하지만 배열에 요소의 개수와 length 프로퍼티의 값이 반드시 일치하지는 않는다.

이러한 배열은 희소 배열이라 한다. 희소 배열은 배열의 요소가 연속적이지 않은 배열을 의미하며 희소 배열이 아닌 일반 배열(C, Java)은 배열의 요소 개수와 length 프로퍼티의 값이 언제나 일치하지만 희소 배열은 배열의 요소 개수보다 length 프로퍼티의 값이 크거나 같다.
{:.info}
