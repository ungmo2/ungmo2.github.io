...
| 간편하게 문자열로 정의 여러 요소를 DOM에 추가할 수 있다.ㅣ 해당 요소의 내용을 덮어 쓴다. 즉, HTML을 로드하고 다시 파싱한다. 이것은 비효율적이다.
| 콘텐츠를 취득할 수 있다.
...
| 장점                             | 단점
|:--------------------------------|:-----------------------------|
| 간편하게 문자열로 정의된 여러 요소를 DOM에 추가할 수 있다.| XSS공격에 취약점이 있기 때문에 사용자로 부터 입력받은 콘텐츠(untrusted data: 댓글, 사용자 이름 등)를 추가할 때 사용해서는 않된다.
| 삽입되는 위치를 선정할 수 있다. |
