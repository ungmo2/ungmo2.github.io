...
section: javascript
description: 텍스트로 만들어져 있는 웹 문서를 브라우저에 렌더링하기 위해서는 웹 문서를 구조화된 객체 모델로 변환한 후, 메모리에 올려야 한다. 이때 메모리에 적재된 구조화된 객체 모델이 바로 DOM이다. 웹 문서를 동적으로 변경하기 위해서 웹 문서 자체를 변경하는 것 보다는 메모리에 적재된 객체 모델을 변경하는 것이 더 나은 방법일 것이다. 따라서 메모리에 적재된 객체 모델 즉 DOM은 프로그래밍 언어가 자신에 접근하고 수정할 수 있는 방법을 제공하는데 일반적으로 프로퍼티와 메서드를 갖는 JavaScript 객체로 제공된다. 이를 DOM API라고 부른다. 정적인 웹페이지에 접근하여 동적으로 웹페이지를 변경하기 위한 유일한 방법은 메모리 상에 존재하는 DOM을 변경하는 것이다. 이때 필요한 것이 DOM에 접근하고 변경하는 프로퍼티와 메서드의 집합인 DOM API이다.
---
...
텍스트로 만들어져 있는 웹 문서를 브라우저에 렌더링하기 위해서는 웹 문서를 구조화된 객체 모델로 변환한 후, 메모리에 올려야 한다. 이때 메모리에 적재된 구조화된 객체 모델이 바로 DOM이다.

웹 문서를 동적으로 변경하기 위해서 웹 문서 자체를 변경하는 것 보다는 메모리에 적재된 객체 모델을 변경하는 것이 더 나은 방법일 것이다. 따라서 메모리에 적재된 객체 모델 즉 DOM은 프로그래밍 언어가 자신에 접근하고 수정할 수 있는 방법을 제공하는데 일반적으로 프로퍼티와 메서드를 갖는 JavaScript 객체로 제공된다. 이를 DOM API라고 부른다. 정적인 웹페이지에 접근하여 동적으로 웹페이지를 변경하기 위한 유일한 방법은 메모리 상에 존재하는 DOM을 변경하는 것이다. 이때 필요한 것이 DOM에 접근하고 변경하는 프로퍼티와 메서드의 집합인 DOM API(Application Programming Interface)이다.

DOM은 HTML, JavaScript가 정의한 규약의 일부가 아니다. HTML, JavaScript와는 별개의 규약으로 W3C의 공식 표준이며 플랫폼/프로그래밍 언어 중립적이다. DOM은 다음 두 가지 기능을 담당한다.
