...
- 요소의 id 어트리뷰트 값으로 해당 요소를 선택할 수 있다.
...
- CSS 셀렉터를 이용해 요소를 선택할 수 있다. 복수개가 선택된 경우, 첫번째 요소만 반환한다.
...
지정된 class를 가지는 요소를 모두 선택한다. 공백으로 구분하여 여러개의 class 값을 지정할 수 있다.
{: .title}
...
| 어트리뷰트(name=foo)| document.getElementsByName('foo')| document.querySelectorAll('[name=foo])
...
어트리뷰트 노드을 조작할 때 다음 프로퍼티 또는 메서드를 사용할 수 있다.

className
{: .title}

class 값을 취득 또는 변경한다. class 어트리뷰트가 존재하지 않으면 지정된 값으로 class를 생성한다. class 값이 여러개일 경우, 공백으로 구분된 문자열이 반환되므로 String 메서드 `split(' ')`를 사용하여 배열로 변경한다.
...
id 값을 취득 또는 변경한다. id 어트리뷰트가 존재하지 않으면 지정된 값으로 id 어트리뷰트를 생성한다.
...
HTML Contents를 조작(Manipulation)하기 위해 아래의 프로퍼티 또는 메서드를 사용할 수 있다. 마크업이 포함된 Contents를 추가하는 행위는 크로스 스크립팅 공격(XSS: Cross-Site Scripting Attacks)에 취약하므로 주의가 필요하다.
