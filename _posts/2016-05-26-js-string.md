...
첫번째 인수로 전달한 index에 해당하는 문자부터 두번째 인자에 전달된 index에 해당하는 문자의 **바로 이전 문자까지**를 모두 반환한다. 이때 첫번째 인수 < 두번째 인수의 관계가 성립된다.
...
## 3.8 String.prototype.toLowerCase(): string <sup>ES1</sup>
...
## 3.12 String​.prototype​.includes(searchString: string, position?: number): boolean <sup>ES6</sup>
