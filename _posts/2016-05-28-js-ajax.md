...
[XMLHttpRequest.send](https://developer.mozilla.org/en-US/docs/Web/API/XMLHttpRequest/send) 메소드로 준비된 요청을 서버에 전달한다. XMLHttpRequest.send 메소드는 **request body**에 담아 전송할 인수를 사용할 수 있다. 
...
GET 메소드의 경우, URL의 일부분인 쿼리문자열(query string)로 파라미터를 서버로 전송한다.
...
text 타입으로는 text/css, text/javascript, text/html, text/plain등이 있다.

File을 업로드하기 위한 타입으로는 multipart/formed-data가 있다.
