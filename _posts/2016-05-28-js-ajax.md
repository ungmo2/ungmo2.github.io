...
브라우저는 <strong>[XMLHttpRequest](https://developer.mozilla.org/ko/docs/XMLHttpRequest) 객체</strong>를 이용하여 Ajax 요청을 생성하고 전송한다. 서버가 브라우저의 요청에 대해 응답을 반환하면 같은 XMLHttpRequest 객체가 그 결과를 처리한다.
...
[XMLHttpRequest.send](https://developer.mozilla.org/en-US/docs/Web/API/XMLHttpRequest/send) 메소드로 준비된 요청을 서버에 전달한다. XMLHttpRequest.send 메소드는 request body에 담아 전송할 인수를 사용할 수 있다. 만약 **요청 메소드가 GET이면 send 메소드의 인수는 무시되고 request body은 null로 설정된다.**
...
req.open('POST', '/books');
req.setRequestHeader('Content-type', 'application/json');
req.send(JSON.stringify({ title: 'javascript' }));
```

books 리소스의 책의 타이틀을 "javascript"에서 "ecmascript"로 갱신한다.
...
```javascript
var req = new XMLHttpRequest();
req.open('PUT', '/books/2');
req.setRequestHeader('Content-type', 'application/json');
req.send(JSON.stringify({ title: 'ecmascript' }));
```
