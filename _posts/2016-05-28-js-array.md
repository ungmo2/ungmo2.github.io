...
배열은 객체이기 때문에 배열의 요소를 삭제하기 위해 `delete` 연산자를 사용할 수 있다. 이때 length에는 변함이 없다. 해당 요소를 완전히 삭제하여 length에도 반영되게 하기 위해서는 [Array.prototype.splice](./js-array#510-arrayprototypesplicestart-number-deletecountthislength-start-items-t-t-%EF%B8%8F-es3) 메소드를 사용한다.
...
length 프로퍼티는 요소의 개수(배열의 길이)를 나타낸다. 배열 인덱스는 32bit 양의 정수로 처리된다. 따라서 length 프로퍼티의 값은 양의 정수이며 2<sup>32</sup>(4,294,967,296) 미만이다.

하지만 배열에 요소의 개수와 length 프로퍼티의 값이 반드시 일치하지는 않는다.

배열에 요소의 개수와 length 프로퍼티의 값이 일치하지 않는 배열을 **희소 배열**(sparse array)이라 한다. 희소 배열은 배열의 요소가 연속적이지 않은 배열을 의미한다. 희소 배열이 아닌 일반 배열은 배열의 요소 개수와 length 프로퍼티의 값이 언제나 일치하지만 희소 배열은 배열의 요소 개수보다 length 프로퍼티의 값이 언제나 크다. 희소 배열은 일반 배열보다 느리고 메모리를 더 많이 사용한다.
{:.info}

현재 length 프로퍼티보다 더 큰 인덱스로 요소를 추가하면 length 프로퍼티는 새로운 요소를 추가할 수 있도록 자동으로 늘어난다. 즉, length 프로퍼티는 가장 큰 인덱스에 1을 더한 것과 같다.
...
console.log(myArray);        // (1001) [empty × 1000, true]
console.log(myArray.length); // 1001
console.log(myArray[0]);     // undefined
```

length 프로퍼티는 명시적으로 값을 변경할 수 있다. 만약 length 프로퍼티의 값을 현재보다 작게 설정하면 설정한 값보다 크거나 같은 인덱스에 해당하는 요소는 모두 삭제된다.
...
// 배열 길이의 명시적 설정
arr.length = 3; // (3) ["zero", "one", "two"]

// 배열 끝에 새 요소 추가
arr[arr.length] = 'san'; // (4) ["zero", "one", "two", "san"]

arr.length = 5; // (5) ["zero", "one", "two", "san", empty]

// 배열 끝에 새 요소 추가
arr.push('go'); // (6) ["zero", "one", "two", "san", empty, "go"]
```
