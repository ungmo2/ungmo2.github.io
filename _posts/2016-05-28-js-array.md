...
이러한 배열은 **희소 배열**(sparse array)이라 한다. 희소 배열은 배열의 요소가 연속적이지 않은 배열을 의미하며 희소 배열이 아닌 일반 배열(C, Java)은 배열의 요소 개수와 length 프로퍼티의 값이 언제나 일치하지만 희소 배열은 배열의 요소 개수보다 length 프로퍼티의 값이 크거나 같다.
{:.info}

현재 length 프로퍼티보다 더 큰 인덱스로 항목을 추가하면 length 프로퍼티는 새로운 항목을 추가할 수 있도록 자동으로 늘어난다. 즉, length 프로퍼티는 가장 큰 인덱스에 1을 더한 것과 같다.
