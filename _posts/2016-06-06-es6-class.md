...
자바스크립트는 ES6의 class를 포함하여 모든 선언(var, let, const, function, function*, class)을 호이스팅(Hoisting)한다.
...
// var 키워드로 선언한 변수 호이스팅
bar(); // TypeError: foo is not a function
var bar = function () { console.log('hi'); }; 

// let, const 키워드로 선언한 변수 호이스팅
baz(); // ReferenceError: foo is not defined
const baz = function () { console.log('hi'); };
```

class 선언문 이전에 class를 참조하면 ReferenceError가 발생한다. 이는 스코프의 선두에서 class의 선언까지 <strong>일시적 사각지대(Temporal Dead Zone; TDZ)</strong>에 빠지게 되기 때문이다.
...
(A) super 메서드는 자식 class의 constructor 내부에서 부모 class의 constructor(super-constructor)를 호출한다. 자식 클래스에서 constructor를 생략하지 않는 경우, 자신의 constructor 내에서 반드시 super()를 호출하여야 한다.
