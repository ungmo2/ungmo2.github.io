...
지금부터 살펴볼 예제는 현재 대부분의 브라우저가 ES6의 모듈을 지원하지 않고 있으므로 동작하지 않는다. 이 문제에 대해서는 [Babel 6와 Webpack 4를 이용한 ES6 개발환경 구축](./es6-babel)에서 알아볼 것이다. 이 장에서는 ES6 모듈의 기본 문법에 대해서만 살펴보도록 하자.
{: .info}
