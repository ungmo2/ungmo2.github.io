...
[XMLHttpRequest](https://developer.mozilla.org/ko/docs/XMLHttpRequest) 객체의 인스턴스를 생성하고 open() 메소드를 사용하여 서버에의 요청을 준비한다. send() 메소드로 준비된 요청을 서버에 전달한다.
...
브라우저가 서버로부터 응답을 받으면 onload 이벤트가 발생한다. xhr 객체의 status 프로퍼티값을 검사하여 정상 응답인지 확인한다. 정상 응답이면 이후 처리를 진행한다.
