...
primary-key인 &#95;id는 명시적으로 정의하지 않아도 insert()나 save() 메소드 호출시 자동으로 추가된다.
...
model(위의 경우 User)이 호출한 메소드는 Statics model method(Statics)이다.
...
model의 instance(위의 경우 lee)가 호출한 메소드는 Document instance method(Methods)가 된다.
...
create() 메소드의 첫번째 매개변수에는 object 또는 array를 전달할 수 있다. array에 여러개의 user 정보를 담아 한번에 저장할 수 있어 유용하다.
