...
Angular CLI의 빌드 기능은 TypeScript를 JavaScript로 트랜스파일링한다. 사실은 TypeScript뿐만 아니라 컴포넌트의 템플릿 또한 컴파일이 필요하다. 데이터바인딩과 이벤트를 위한 템플릿은 컴파일을 필요로 하기 때문이다. 하지만 템플릿은 빌드 시에 컴파일되지 않고 코드가 실행될 때 JIT(Just-In-Time)로 컴파일된다.

AoT 컴파일이란 TypeScript의 트랜스파일링과 같이 빌드 시에 컴파일되는 것을 의미한다. 빌드에 소요되는 시간이 조금 더 걸리더라도 코드 실행시 컴파일이 실행되지 않기 때문에 실제 어플리케이션으이 동작 시간은 단축되는 효과가 있다.

또한 JIT 컴파일러를 포함할 필요가 없어지기 때문에 어플리케이션 전체 용량도 줄어드는 효과가 있다.
