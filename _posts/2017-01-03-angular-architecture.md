...
Angular는 프레임워크이므로 웹 어플리케이션의 기본적인 기능을 구현한 정형화된 구조를 가진 구현체를 제공한다. 이것을 바탕으로 작성하고자 하는 어플리케이션에 필요한 기능을 추가하는 방식으로 어플리케이션을 완성해 간다. 따라서 프레임워크의 학습에는 정형화된 프레임워크의 구조에 익숙해지는 것이 필요하다. Angular가 제공하는 기본 구조와 각 파일의 기능을 살펴보도록 하자.
