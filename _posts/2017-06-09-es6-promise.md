...
Promise.allSettled 메소드도 프로미스가 담겨 있는 배열 등의 [이터러블](./es6-iteration-for-of)을 인자로 전달 받는다. 그리고 전달받은 모든 프로미스를 병렬로 처리하고 그 처리 결과를 resolve하는 새로운 프로미스를 반환한다. 하지만 각 프로미스 작업의 성공(fulfilled), 실패(rejected) 유무와 상관없이 완료(settled)되면 반환한다는 점이 Promise.all과 다르다.
...
위 예제는 Promise.all과 동일하게, 전달받은 모든 프로미스를 병렬로 처리될 때까지 기다린 후 아래와 모든 처리 결과를 resolve한다.
...
Promise.allSettled([
  ...
  new Promise((resolve, reject) => setTimeout(() => reject(new Error('Error 2!')), 2000)),
  new Promise((resolve, reject) => setTimeout(() => reject(new Error('Error 3!')), 1000))
]).then(console.log) // [{status: "fulfilled", value: 1}, {status: "rejected", reason: Error: Error 2!}, {status: "rejected", reason: Error: Error 3!}]
  .catch(console.log);
```

하지만 위 예제의 경우, 세번째 프로미스가 가장 먼저 실패하지만 첫번째 프로미스가 완료될 때까지 기다린 후 then 메소드로 전달된다.

그리고 then 메소드에서 결과를 보면, 각 프로미스의 상턔(status)와 결과(value)를 반환하는 것을 확인할 수 있다.
