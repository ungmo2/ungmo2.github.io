...
2021년 10월 10일 출시된 eslint 8.0.0을 설치한 경우 [eslint-plugin-import가 정상 동작하지 않는 버그](https://github.com/eslint/eslint/issues/15148)가 있다. 이때 `npm install --save-dev eslint@"^7.0.0"`로 eslint 7 버전 중에서 최신 버전을 설치하면 정상 동작한다.
{: .info}
...
: ESLint 설정 중에서 Prettier와 충돌하는 부분을 비활성화 한다. eslint의 버전이 7.0 이상이 아니면 에러가 발생해 설치되지 않는다. 이러한 경우 `npm install --save-dev eslint@"^7.0.0"`으로 eslint 7 버전을 설치하도록 한다.
...
React 프로젝트에서 ESLint와 Prettier를 같이 사용하려면 [eslint-config-prettier] 플러그인을 추가로 로컬 설치해야 한다.
