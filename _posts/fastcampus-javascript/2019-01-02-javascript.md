...
Node.js는 **논블로킹(Non-blocking) I/O**와 **단일 스레드 이벤트 루프**를 통한 높은 요청(Request) 처리 성능을 가지고 있다. 예를 들어, 데이터베이스로부터 대량의 데이터를 취득하여 웹페이지에 표시할 때 데이터베이스 처리에 대기시간(blocking)이 발생하기 때문에 웹페이지 표시가 지연되는 현상이 발생한다.

Node.js의 모든 API는 **비동기(asynchronous) 방식**으로 동작하여 논블로킹(Non-blocking) I/O가 가능하고 단일 스레드 이벤트 루프 모델을 사용하여 보다 가벼운 환경에서도 높은 Request 처리 성능을 가지고 있다.
