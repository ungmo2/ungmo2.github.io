...
렌더링
: 렌더링(Rendering)이란 HTML과 CSS로 표현된 데이터를 해석하여 시각적으로 브라우저에 표현하는 것을 말한다. 때로는 데이터를 HTML로 변환하는 것을 가리키기도 한다. 브라우저가 HTML과 CSS를 로드하고 해석하여 렌더링하는 과정은 "3.2.1 웹 브라우저는 어떻게 동작하는가?"에서 살펴볼 것이다.
