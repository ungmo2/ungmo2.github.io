...
위 예제의 경우, 자바스크립트 엔진은 표현식 `x + ''`을 평가하기 위해 변수 x의 숫자 값을 바탕으로 새로운 문자열 값 '10'을 생성하고 이것으로 표현식 `'10' + ''`를 평가한다. 이때 자동 생성된 문자열 '10'은 변수 x에 할당되는 것이 아니다. 따라서 자동 생성된 문자열 '10'은 표현식의 평가가 끝나면 아무도 참조하지 않으므로 가비지 컬렉터에 의해 메모리에서 제거된다.
