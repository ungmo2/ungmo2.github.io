...
"4.3 변수 선언의 실행 시점과 변수 호이스팅"에서 살펴보았듯이 변수 선언은 다른 코드가 실행되기 이전에 변수 선언이 어디에 있던지 상관없이 가장 먼저 실행된다. 다시 말해, 변수 선언은 코드가 한 줄씩 순차적으로 실행되는 시점인 런타임(runtime)에 실행되는 것이 아니라 런타임 이전인 구문 분석(Syntax analysis) 단계에 자바스크립트 엔진에 의해 먼저 실행된다.
