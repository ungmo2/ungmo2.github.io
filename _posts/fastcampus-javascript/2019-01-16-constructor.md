...
객체는 프로퍼티를 통해 객체 고유의 상태(state)를 표현한다. 그리고 메소드를 통해 상태 데이터인 프로퍼티를 참조하고 조작하는 동작(behavior)을 표현한다. 따라서 프로퍼티는 객체마다 프로퍼티 값이 다를 수 있지만 메소드는 일반적으로 동일한 경우가 많다.
...
일반적으로 프로퍼티에 값인 함수는 모두 메소드로 통칭한다. 하지만 ECMAScript 사양에서 "메소드 정의"란 ES6의 메소드 축약 표현만을 의미한다. 다시 말해 함수가 어디에 할당되어 있는지에 따라 메소드 정의인지를 판단하는 것이 아니라 함수 정의 방식에 따라 함수의 종류를 구분한다.
...
먼저 생성자 함수의 함수 몸체에서 수행해야 하는 것이 무엇인지 생각해보자. 생성자 함수의 역할은 프로퍼티 구조가 동일한 인스턴스를 생성하기 위한 템플릿(클래스)으로서 동작하여 인스턴스를 생성하는 것과 생성된 인스턴스의 프로퍼티 값을 초기화하는 것이다. 생성자 함수가 인스턴스를 생성하는 것은 필수이고, 생성된 인스턴스의 프로퍼티 값을 초기화하는 것은 옵션이다. 아래 예제를 살펴보자.

```javascript
// 생성자 함수
function Circle(radius) {
  // 생성자 함수가 생성할 인스턴스, 즉 this에 프로퍼티를 추가한다.
  // 프로퍼티에 인수로 전달받은 초기값을 할당한다.
  this.radius = radius;
  this.getDiameter = function () {
    return 2 * this.radius;
  };
...
생성자 함수 내부의 코드를 살펴보면 인스턴스를 생성하고 반환하는 코드는 보이지 않는다. 다만 this에 프로퍼티를 추가하고 필요에 따라 인수로 전달된 초기값을 프로퍼티 값으로 할당한다.

자바스크립트 엔진은 암묵적인 처리를 통해 인스턴스를 생성하고 반환한다. new 연산자와 함께 생성자 함수가 호출되면 자바스크립트 엔진은 아래와 같은 과정을 거쳐 인스턴스를 생성하고 프로퍼티를 초기화한 후, 인스턴스를 반환한다.

1. 빈 객체 생성과 this 바인딩
생성자 함수 내부의 코드가 실행되기 전에 먼저 빈 객체가 암묵적으로 생성된다. 그리고 생성된 빈 객체는 this에 바인딩된다.
...
하지만 명시적으로 원시 타입(primitive type)의 값을 반환하면 원시 타입의 반환은 무시되고 this가 반환된다.
...
function Circle(radius) {
  ...
  // 3. 암묵적으로 this를 반환한다.
  // 명시적으로 원시 타입의 값을 반환하면 원시 타입의 반환은 무시되고 암묵적으로 this가 반환된다.
  return 100;
}
...
let inst = new add();
// 함수가 객체를 반환하지 않으면 빈 객체가 생성되어 반환된다.
console.log(inst); // {}
...
Circle 함수를 new 연산자와 함께 생성자 함수로서 호출하면 함수 내부의 this는 Circle 생성자 함수가 생성할 인스턴스를 가리킨다. 하지만 Circle 함수를 일반적인 함수로서 호출하면 함수 내부의 this는 전역 객체 window를 가리킨다. 따라서 일반 함수로서 호출된 위 예제의 Circle 함수 내부의 this는 전역 객체 window를 가리킨다. radius 프로퍼티와 getDiameter 메소드는 전역 객체의 프로퍼티와 메소드가 된다. (“20. 전역 객체” 참고)
...
new.target은 함수 내부에서 지역 변수와 같이 사용되며 메타 프로퍼티([meta propery](https://tc39.github.io/ecma262/#sec-meta-properties))라고 부른다. IE는 new.target을 지원하지 않으므로 주의하기 바란다.

함수 내부에서 new.target를 사용하면 함수가 new 연산자와 함께 호출되었는지 여부를 확인할 수 있다. 함수가 new 연산자와 함께 호출되면 함수 내부의 new.target은 함수 자신을 가리킨다. new 연산자 없이 호출된 함수 내부의 new.target은 undefined이다.
