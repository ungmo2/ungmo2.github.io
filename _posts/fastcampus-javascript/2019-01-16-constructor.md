...
1. 인스턴스 생성과 this 바인딩
암묵적으로 빈 객체가 생성된다. 이때 생성된 빈 객체가 바로 (아직 완성되진 않았지만) 생성자 함수가 생성한 인스턴스이다. 이때 생성된 인스턴스의 프로토타입으로 생성자 함수의 prototype 프로퍼티가 가리키는 객체가 설정된다. 그리고 암묵적으로 생성된 빈 객체, 즉 인스턴스는 this에 바인딩된다.

그 후, 암묵적으로 생성된 빈 객체, 즉 인스턴스는 this에 바인딩된다. 그리고 인스턴스의 프로토타입으로 생성자 함수의 prototype 프로퍼티가 가리키는 객체가 설정된다.
...
생성자 함수 내부의 모든 처리가 끝나면 암묵적으로 this, 즉 인스턴스를 반환한다.
