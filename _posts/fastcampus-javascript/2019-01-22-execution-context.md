...
if 문이 실행되면 if 문의 블록 레벨 스코프를 생성해야 한다. 이를 위해 if 문의 렉시컬 환경을 생성하고 실행 중인 실행 컨텍스트의 렉시컬 환경으로 교체한다.
...
if 문 실행이 종료되면 실행 중인 실행 컨텍스트의 렉시컬 환경을 if 문이 실행되기 이전의 렉시컬 환경으로 되돌린다.
