...
무슨 의미인지 잘 와닿지 않는다. 정의가 무척이나 난해하니 예제를 통해 정의의 의미부터 살펴보자. 위 정의에서 중요한 키워드는 "함수가 선언된 렉시컬 환경(Lexical environment)"이다.
...
자바스크립트는 함수를 어디서 호출했는지가 아니라 어디에 정의했는지에 따라 상위 스코프를 결정한다. 이를 렉시컬 스코프(정적 스코프)라 한다. 그리고 함수는 자신이 존재하는 한 자신이 정의되었을 당시의 상위 스코프를 기억한다.

"22. 실행 컨텍스트"에서 살펴보았듯이 스코프의 실체는 실행 컨텍스트의 렉시컬 환경이다. 이 렉시컬 환경은 자신의 "외부 렉시컬 환경에 대한 참조(Outer Lexical Environment Reference)"를 통해 상위 렉시컬 환경과 연결된다. 이것이 바로 스코프 체인이다.

따라서 "함수가 상위 스코프를 결정한다"는 것은 "렉시컬 환경의 외부 렉시컬 환경에 대한 참조에 저장할 참조값을 결정한다"는 것과 같다. 렉시컬 환경의 "외부 렉시컬 환경에 대한 참조"에 저장할 참조값이 바로 상위 렉시컬 환경에 대한 참조이며 이것이 상위 스코프이다.

렉시컬 환경의 "외부 렉시컬 환경에 대한 참조"에 저장할 참조값, 즉 상위 스코프에 대한 참조는 함수 정의 시점에 결정된다. 이것이 바로 렉시컬 스코프이다.

모든 함수는 함수 정의가 평가되어 함수 객체를 생성할 때, 현재 실행중인 실행 컨텍스트의 렉시컬 환경을 함수 객체의 내부 슬롯 [[[Environment]]](https://tc39.github.io/ecma262/#sec-functioninitialize )에 저장한다.

예를 들어, 전역에서 정의된 함수 선언문은 전역 코드가 평가되는 시점에 평가되어 함수 객체를 생성한다. 이때 생성된 함수 객체의 내부 슬롯 `[[Environment]]`에는 함수 정의가 평가된 시점, 즉 전역 코드 평가 시점에 실행중인 실행 컨텍스트의 렉시컬 환경인 전역 렉시컬 환경의 참조가 저장된다.

함수 내부에서 정의된 함수 선언문은 외부 함수 코드가 평가되는 시점에 평가되어 함수 객체를 생성한다. 이때 생성된 함수 객체의 내부 슬롯 `[[Environment]]`에는 함수 정의가 평가된 시점, 즉 외부 함수 코드 평가 시점에 실행중인 실행 컨텍스트의 렉시컬 환경인 외부 함수 렉시컬 환경의 참조가 저장된다.

함수 객체의 내부 슬롯 `[[Environment]]`에 저장한 렉시컬 환경의 참조가 바로 상위 스코프로서 렉시컬 환경의 "외부 렉시컬 환경에 대한 참조"에 저장될 참조값이다. 이를 통해 함수는 내부 슬롯 `[[Environment]]`에 저장한 렉시컬 환경의 참조, 즉 상위 스코프를 자신이 존재하는 한 기억한다.

"12.7 렉시컬 스코프"에서 살펴보았던 아래 예제를 다시 한번 살펴보자.
...
그림 23 1 함수 객체의 내부 슬롯 `[[Environment]]`에는 상위 스코프가 저장된다.
...
그림 23 6 상위 스코프의 식별자를 참조하지 않지 않으면 상위 스코프를 기억하지 않는다.
...
그림 23 7 중첩 함수 inner는 클로저이긴 하지만 외부 함수와 더불어 소멸되기 때문에 상위 스코프를 기억한다는 클로저로써의 역할에 부합하지 않는다.
...
그림 23 8 상위 스코프의 식별자 중 클로저가 참조하고 있는 식별자만을 기억한다.
