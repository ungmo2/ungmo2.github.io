...
배열(array)은 자바스크립트가 제공하는 자료구조이다. 자바스크립트 배열은 사용 빈도가 매우 높은 핵심 자료 구조이다. 자바스크립튼 배열을 다루기 위한 유용한 메소드를 다수 제공한다.

배열은 순서가 있는 값들의 리스트이다. 하나의 변수에 여러 개의 값을 저장할 수 있는 장점이 있다.

배열이 가지고 있는 값을 **요소(element)**라고 부른다. 자바스크립트에서 값으로 인정되는 모든 것이 요소가 될 수 있다.

배열의 요소는 배열에서 자신의 위치를 나타내는 정수인 **인덱스(index)**를 갖는다. 이를 통해 요소에 접근할 있다. 대부분의 프로그래밍 언어에서 인덱스는 0으로 시작한다.
...
특정 인덱스의 요소에 접근할 때는 대괄호 표기법을 사용한다.
...
배열은 Array 생성자로 생성된 객체이며 배열의 프로토타입 객체는 Array.prototype이다. Array.prototype는 매우 편리한 빌트인 메소드 들을 제공한다.
...
배열의 장점은 처음부터 순차적으로 요소에 접근할 수도 있고, 마지막부터 꺼꾸로 요소에 접근할 수도 있으며 특정 위치부터 순차적으로 요소에 접근할 수도 있다는 것이다. 이는 배열이 인덱스와 length 프로퍼티를 갖기 때문에 가능한 것이다.
...
예를 들어, 위 그림처럼 메모리 주소 1000에서 시작하고 각 요소가 4byte인 배열을 생각해 보자.
...
// 인수로 전달받은 모든 값을 원본 배열의 마지막에 요소로 추가하고 변경된 length 값을 반환한다. let result = arr.push(3, 4);
