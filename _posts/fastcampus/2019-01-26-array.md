...
ES10(ECMAScript 2019)에서 새롭게 도입된 flatMap 메소드는 map을 통해 생성된 새로운 배열을 평탄화한다. 즉, map 메소드와 flat 메소드를 순차적으로 실행하는 효과가 있다. 단, [flat 메소드](/fastcampus/array#814-arrayprototypeflat)처럼 인수를 전달하여 평탄화 깊이를 지정할 수는 없고 1단계만 평탄화한다.
