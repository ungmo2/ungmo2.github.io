...
["11.1.1. 변경 불가능한 값"](/fastcampus/primitive-vs-object#11-변경-불가능한-값)에서 살펴본 바와 같이, **const 키워드는 재할당을 금지할 뿐 "불변(immutable)"을 의미하지는 않는다.** 다시 말해, 새로운 값을 재할당하는 것은 불가능하지만 프로퍼티 동적 생성, 삭제, 프로퍼티 값의 변경을 통해 객체를 변경하는 것은 가능하다. 이때 객체가 변경되더라도 변수에 할당된 참조 값은 변경되지 않는다.
