...
아직 배열에 대해 살펴보지 않았기 때문에 지금은 위 예제를 이해하지 못해도 좋다. ["27. 배열"](/fastcampus/array)에서 다시 자세히 살펴보도록 하자. 참고로 ES6의 ["36. 디스트럭처링 할당"](/fastcampus/destructuring)에서도 이에 대해 언급할 것이다.

Function.prototype.bind 메서드는 apply와 call 메서드와 달리 함수를 호출하지 않는다. 다만 첫번째 인수로 전달한 값으로 this 바인딩이 교체된 함수를 [새롭게 생성해 반환](https://developer.mozilla.org/ko/docs/orphaned/Web/JavaScript/Reference/Global_Objects/Function/bind#설명)한다.
