...
현재 날짜와 시간은 자바스크립트 코드가 실행된 시스템의 시계에 의해 결정된다. 시스템 시계의 설정(timezone, 시간)에 따라 서로 다른 값을 가질 수 있다.
...
Date 생성자 함수에 연, 월, 일, 시, 분, 초, 밀리초를 의미하는 숫자를 인수로 전달하면 지정된 날짜와 시간을 나타내는 Date 객체를 반환한다. 이때 연, 월은 반드시 지정하여야 한다. 지정하지 않은 옵션 정보는 0 또는 1으로 초기화된다. 인수는 다음과 같다.
...
Date.UTC 메서드는 `new Date(year, month[, day, hour, minute, second, millisecond])`와 같은 형식의 인수를 사용해야 한다. Date.UTC 메서드의 인수는 local time(KST)가 아닌 UTC로 인식된다. month는 월을 의미하는 0~11까지의 정수다. 0부터 시작하므로 주의가 필요하다.
...
Date 객체의 요일(0 ~ 6)를 나타내는 정수를 반환한다. 반환값은 다음와 같다.
...
```javascript
const today = new Date();
// -> Mon Jul 06 2020 03:11:00 GMT+0900 (대한민국 표준시)

today.getTime(); // -> 1593972660839
```
...
UTC와 Date 객체에 지정된 로케일(locale) 시간과의 차이를 분 단위로 반환한다. KST는 UTC에 9시간을 더한 시간이다. 즉, UTC = KST - 9h다.

```javascript
const today = new Date(); // today의 지정 로케일은 KST다.

//UTC와 today의 지정 로케일 KST와의 차이는 -9시간이다.
...
인수로 전달한 로케일을 기준으로 Date 객체의 날짜와 시간을 표현한 문자열을 반환한다. 인수를 생략한 경우 브라우저가 동작 중인 시스템의 로케일을 적용한다.
...
인수로 전달한 로케일을 기준으로 Date 객체의 시간을 표현한 문자열을 반환한다. 인수를 생략한 경우 브라우저가 동작 중인 시스템의 로케일을 적용한다.
...
(function printNow() {
  ...
  // 1초마다 printNow 함수를 재귀 호출한다. "41.2.1. setTimeout / clearTimeout" 참고
