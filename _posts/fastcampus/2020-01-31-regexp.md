...
정규표현식을 사용하는 메서드는 RegExp.prototype.exec, RegExp.prototype.test, String.prototype.match, String.prototype.replace, String.prototype.search, String.prototype.split 등이 있다. 이 중에서 RegExp.prototype.exec, RegExp.prototype.test, String.prototype.match 메서드에 대해 살펴보자. String.prototype.replace, String.prototype.search, String.prototype.split 메서드는 ["32. String"](/fastcampus/date)에서 살펴볼 것이다.
...
exec 메서드는 g 플래그를 지정하여도 첫번째 매칭 결과만을 반환한다.
...
`?`는 앞선 패턴이 최대 한번(0번 포함) 이상 반복되는 문자열을 의미한다. 즉, `?`는 `{0,1}`과 같다. 아래 예제의 경우, `A?`는 'A'가 최대 한번(0번 포함) 이상 반복되는 문자열('', 'A)을 의미한다.
