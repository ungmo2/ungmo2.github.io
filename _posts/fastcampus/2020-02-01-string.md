...
위 예제를 크롬 브라우저의 개발자 도구에서 실행해보면 [[PrimitiveValue]]라는 프로퍼티가 보인다. 이는 [[StringData]] 내부 슬롯을 가리킨다. ES5에서는 [[StringData]]를 [[PrimitiveValue]]이라 불렀다.
{: .info}
...
["11.1.2. 문자열과 불변성"](/fastcampus/primitive-vs-object#12-문자열과-불변성)에서 살펴보았듯이 문자열은 유사 배열 객체이면서 이터러블이다. 따라서 배열과 유사하게 인덱스를 사용하여 각 문자에 접근할 수 있다.
...
String.prototype.replace 메소드에 [정규 표현식](/fastcampus/regexp)을 인수로 전달하여 공백 문자를 제거할 수도 있다.
