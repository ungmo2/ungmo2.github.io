...
공백 텍스트 노드
: 정확히 말하자면 위 그림에는 공백 텍스트 노드가 생략되어 있다. HTML 요소 사이의 개행이나 공백은 텍스트 노드가 된다. 이에 대해서는 ["39.3.1 공백 텍스트 노드"](/fastcampus/dom#31-공백-텍스트-노드)에서 자세히 살펴보자.
...
위 4가지 노드 타입 이외에도 주석을 위한 Comment 노드, DOCTYPE을 위한 DocumentType 노드, 복수의 노드를 생성하여 추가할 때 사용하는 DocumentFragment 노드 등 총 12개의 노드 타입이 있다. DocumentFragment 노드에 대해서는 ["39.6.4. 복수의 노드 생성과 추가"](/fastcampus/dom#64-복수의-노드-생성과-추가)에서 자세히 살펴보자.
...
querySelectorAll이 반환하는 NodeList 객체는 NodeList.prototype.forEach 메서드를 상속받아 사용할 수 있다. NodeList.prototype.forEach 메서드는 Array.prototype.forEach 메서드와 사용법이 동일하다. NodeList.prototype은 forEach 외에도 item, entries, keys, values 메서드를 제공한다.
