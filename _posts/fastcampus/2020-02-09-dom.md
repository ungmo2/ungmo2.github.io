...
<html>
  <body>
    ...
    <script>
      ...
      /* #fruits 요소의 자손 노드 중에서 class 값이 'banana'인 요소 노드를 모두
      탐색하여 반환한다. */
      const $fruits = document.getElementById('fruits');
...
<html>
  ...
  <script>
    ...
    /* $fruits 요소의 자식 노드는 공백 텍스트 노드(39.3.1절 "공백 텍스트 노드" 참고)를
    포함해 모두 5개다. */
...
| 프로퍼티          | 설명
|:----------------|:------------
| Node.prototype.childNodes | 자식 노드를 모두 탐색하여 DOM 컬렉션 객체인 NodeList에 담아 반환한다. **childNodes 프로퍼티가 반환한 NodeList에는 요소 노드뿐만 아니라 텍스트 노드고 포함되어 있을 수 있다.**
...
<html>
  ...
  <script>
    ...
    childNodes 프로퍼티가 반환한 NodeList에는 요소 노드뿐만 아니라 텍스트 노드도
    포함되어 있다. */
...
<html>
  ...
  <script>
    ...
    HTML5는 innerHTML 프로퍼티로 삽입된 script 요소 내의 자바스크립트 코드를
    실행하지 않는다. */
...
이 과정에서 비로소 새롭게 생성한 요소 노드가 DOM에 추가된다. 기존의 DOM에 요소 노드를 추가하는 처리는 이 과정 뿐이다. 위 예제는 단 하나의 요소 노드를 생성하여 DOM에 한번 추가하므로 DOM은 한 번 변경된다. 이때 [리플로우와 리페인트](/fastcampus/browser-rendering#7-리플로우와-리페인트)가 실행된다.
...
<body>
  ...
  <script>
    ...
    /* getAttribute 메서드로 취득한 HTML 어트리뷰트 값, 즉 초기 상태 값은
    변하지 않고 유지된다. */
...
<body>
  ...
  <script>
    ...
    // user-id가 7621인 요소 노드를 취득한다.
    const user = users.find(user => user.dataset.userId === '7621');
    // user-id가 7621인 요소 노드에서 data-role의 값을 취득한다.
    console.log(user.dataset.role); // "admin"

    // user-id가 7621인 요소 노드의 data-role 값을 변경한다.
...
<body>
  ...
  <script>
    ...
    // user-id가 7621인 요소 노드에 새로운 data 어트리뷰트를 추가한다.
...
`.`으로 시작하는 클래스 선택자를 사용하여 CSS class를 미리 정의한 다음, HTML 요소의 class 어트리뷰트 값을 변경하여 HTML 요소의 스타일을 변경할 수도 있다. 이때 HTML 요소의 class 어트리뷰트를 조작하려면 ["39.7. 어트리뷰트"](/fastcampus/dom#7-어트리뷰트)에서 살펴보았듯이 class 어트리뷰트에 대응하는 요소 노드의 DOM 프로퍼티를 사용한다.
...
style 프로퍼티는 인라인 스타일만을 반환한다. 따라서 클래스를 적용한 스타일이나 상속을 통해 암묵적으로 적용된 스타일은 style 프로퍼티로 참조할 수 없다. HTML 요소에 적용되어 있는 모든 CSS 스타일을 참조해야 할 경우 getComputedStyle 메서드를 사용한다.
