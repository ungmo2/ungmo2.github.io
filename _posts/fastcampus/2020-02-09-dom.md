...
공백 텍스트 노드
: 정확히 말하자면 위 그림에는 공백 텍스트 노드가 제외되어 있다. 태그 사이의 개행이나 공백은 텍스트 노드가 된다. 이에 대해서는 "38.3.1 공백 텍스트 노드"에서 살펴보도록 하자.
...
위 4가지 노드 이외에도 주석을 위한 Comment 노드, DOCTYPE을 위한 DocumentType 노드, 복수의 노드를 생성하여 추가할 때 사용하는 DocumentFragment 노드 등 총 12개의 타입이 있다. DocumentFragment 노드에 대해서는 "38.6.4. 복수의 노드 생성과 추가"에서 살펴볼 것이다.
