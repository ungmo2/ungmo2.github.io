...
프로미스의 상태 정보는 resolve 또는 reject 함수를 호출하는 것으로 결정된다. resolve 또는 reject 함수를 호출할 때 전달한 비동기 처리 결과 또는 에러는 프로미스의 후속 처리 메서드에 전달된다.

프로미스의 상태 정보는 resolve 또는 reject 함수를 호출하는 것으로 결정된다. resolve 또는 reject 함수를 호출할 때 전달한 비동기 처리 결과 또는 에러는 프로미스의 후속 처리 메서드에 전달된다.

프로미스는 비동기 처리 상태 정보와 더불어 비동기 처리 결과 정보도 상태로 갖는다. 아래의 프로미스를 개발자 도구에서 출력해보자.
...
fulfilled된 프로미스는 비동기 처리 상태 정보인 "fulfilled"와 비동기 처리 결과 정보인 1을 갖는다.
...
rejected된 프로미스는 비동기 처리 상태 정보인 "rejected"와 비동기 처리 결과 정보인 Error를 갖는다.

# 3. 프로미스의 후속 처리 메서드

프로미스를 사용해 구현한 비동기 함수는 프로미스를 반환해야 한다. 프로미스를 사용해 구현한 비동기 함수를 호출하는 측(promise consumer)에서는 Promise 객체의 후속 처리 메서드인 then, catch, finally를 통해 비동기 처리 결과 또는 에러를 전달받아 후속 처리를 수행한다. 모든 후속 처리 메서드 또한 비동기로 동작한다.

프로미스는 비동기 처리가 성공(fulfilled)했는지 또는 실패(rejected)했는지 등의 상태 정보를 갖는다고 했다. 프로미스는 상태 정보에 따라 후속 처리 메서드를 체이닝 방식으로 호출한다. 프로미스의 후속 처리 메서드는 다음과 같다.

## 3.1. Promise.prototype.then

then 메서드는 두 개의 콜백 함수를 인수로 전달받는다.

- 첫 번째 콜백 함수는 프로미스가 fulfilled 상태(resolve 함수가 호출된 상태)가 되면 호출된다. 이때 콜백 함수는 인수로 프로미스의 처리 결과를 전달받는다.
- 두 번째 콜백 함수는 프로미스가 rejected 상태(reject 함수가 호출된 상태)가 되면 호출된다. 이때 콜백 함수는 인수로 프로미스의 에러를 전달받는다.
...
위 예제에서 then ➝ then ➝ catch 순서로 후속 처리 메서드가 호출되었다. then, catch, finally 후속 처리 메서드는 언제나 프로미스를 반환하므로 연속적으로 호출할 수 있다. 이를 프로미스 체이닝이라 한다.
...
Promise.all 메서드는 인수로 전달받은 배열의 모든 프로미스가 모두 fulfilled 상태가 되면 종료한다. 따라서 Promise.all 메서드가 종료하는 데 걸리는 시간은 가장 늦게 fulfilled 상태가 되는 프로미스의 처리 시간보다 조금 더 길다. 위 예제의 경우 모든 처리에 걸리는 시간은 가장 늦게 fulfilled 상태가 되는 첫 번째 프로미스의 처리 시간 3초보다 조금 더 소요된다.
...
Promise.allSettled 메서드는 프로미스를 요소로 갖는 배열 등의 이터러블을 인수로 전달받는다. 그리고 전달받은 프로미스가 모두 settled 상태가 되면 처리 결과를 배열로 반환한다. ES11(ECMAScript 2020)에 도입된 Promise.allSettled 메서드는 IE를 제외한 대부분의 모던 브라우저에서 지원하고 있다. 다음 예제를 살펴보자.
...
-	프로미스가 fulfilled 상태인 경우 처리 상태를 나타내는 status 프로퍼티와 처리 결과를 나타내는 value 프로퍼티를 갖는다.
-	프로미스가 rejected 상태인 경우 처리 상태를 나타내는 status 프로퍼티와 에러를 나타내는 reason 프로퍼티를 갖는다.
