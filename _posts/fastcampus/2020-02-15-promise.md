...
Promise.all 메서드는 Promise 객체를 요소로 갖는 배열 등의 이터러블을 인자로 전달받는다. 그리고 전달받은 모든 Promise 객체가 모두 resolve 또는 첫 번째 reject되면 그 처리 결과를 resolve 또는 reject하는 새로운 Promise 객체를 반환한다. 다음 예제를 살펴보자.
...
]).then(console.log) // [ 1, 2, 3 ] => 처리 시간은 3초보다 조금 더 소요된다.
  .catch(console.error);
```
...
Promise.all은 배열 내 모든 Promise 객체의 resolve 또는 첫 번째 reject를 기다린다. 즉, Promise.all 메서드는 전달받은 모든 Promise 객체가 완료(모두 resolve 또는 첫 번째 reject)될 때 완료된다. 따라서 처리 완료에 걸리는 시간은 가장 늦게 완료하는 Promise 객체의 처리 시간보다 조금 더 길다. 위 예제의 경우 처리 시간은 가장 늦게 완료되는 첫 번째 Promise 객체의 처리 시간인 3초보다 조금 더 소요된다.
...
위 예제의 Promise.all 메서드는 promiseGet 함수가 반환한 3개의 Promise 객체의 배열을 인수로 전달받고 이 Promise 객체들이 모두 resolve 또는 첫 번째 reject되면 그 처리 결과를 resolve 또는 reject하는 새로운 Promise 객체를 반환한다. 모든 Promise 객체의 처리가 성공하면 Promise.all 메서드는 각 Promise 객체가 resolve한 3개의 사용자 객체가 담긴 배열을 resolve하는 새로운 Promise 객체를 반환하고 후속 처리 메서드 then에는 3개의 사용자 객체가 담긴 배열이 전달된다.
...
Promise.allSettled 메서드는 Promise 객체를 요소로 갖는 배열 등의 이터러블을 인자로 전달받는다. 그리고 전달받은 모든 Promise 객체가 모두 완료되면 fulfilled 또는 rejected 상태와는 상관없이 결과를 배열로 반환한다. ES11(ECMAScript 2020)에 새롭게 도입된 Promise.allSettled 메서드는 IE를 제외한 대부분의 브라우저에서 지원하고 있다. 다음 예제를 살펴보자.
