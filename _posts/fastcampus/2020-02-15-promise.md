...
프로미스는 다음과 같이 현재 비동기 처리가 아땋게 진행되고 있는지를 나타내는 상태(state) 정보를 갖는다.
...
fulfilled된 프로미스는 비동기 처리 상태인 "fulfilled"와 비동기 처리 결과인 1을 갖는다.
...
rejected된 프로미스는 비동기 처리 상태인 "rejected"와 비동기 처리 결과인 Error 객체를 갖는다.

resolve 또는 reject 함수를 호출할 때 전달한 비동기 처리 결과는 프로미스의 후속 처리 메서드의 콜백 함수의 인수로 전달된다.
...
프로미스는 pending, fulfilled, rejected 같은 비동기 처리 상태를 갖는다고 했다. 프로미스는 비동기 처리 상태에 따라 후속 처리 메서드의 콜백 함수를 호출한다. 프로미스의 후속 처리 메서드는 다음과 같다.
...
Promise.allSettled 메서드는 프로미스를 요소로 갖는 배열 등의 이터러블을 인수로 전달받는다. 그리고 전달받은 프로미스가 모두 settled 상태(fulfilled 또는 rejected와 상관 없이 pending이 아닌 상태, 즉 비동기 처리가 수행된 상태)가 되면 처리 결과를 배열로 반환한다. ES11(ECMAScript 2020)에 도입된 Promise.allSettled 메서드는 IE를 제외한 대부분의 모던 브라우저에서 지원하고 있다. 다음 예제를 살펴보자.
